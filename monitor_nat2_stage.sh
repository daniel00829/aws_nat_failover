...
NAT_BACKUP_ID="`aws ec2 describe-instances --filters 'Name=tag:Name,Values=NAT2' --query "Reservations[*].Instances[*].{Instance:InstanceId}"  --output text`"
NAT_BACKUP_PRIVATE_IP="`aws ec2 describe-instances --filters 'Name=tag:Name,Values=NAT2' --query "Reservations[*].Instances[*].{PrivateIP:PrivateIpAddress}"  --output text`"
